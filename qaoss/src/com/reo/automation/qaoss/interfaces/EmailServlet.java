...
import com.reo.automation.qaoss.interfaces.service.EmailService;
 ...
 * @author timen.xu
 */
@WebServlet("/interface/email/*")
public class EmailServlet extends BaseServlet {
    @Resource
    private EmailService emailService;
    
    @AuthIgnore
    @WebAction(url = "/interface/email/queryall", actionid = ACTION_QUERY)
    public void queryAll(HttpRequest req, HttpResponse resp) { 
        EmailFilter bean = req.getJsonParameter(EmailFilter.class, "bean");
        Flipper flipper = req.getFlipper();
        flipper.setSort("ctime desc");
        resp.finishJson(emailService.querySheet(flipper, bean));
    } 
    
    @AuthIgnore
    @WebAction(url = "/interface/email/queryList", actionid = ACTION_QUERY)
    ...
    @WebAction(url = "/interface/email/update", actionid = ACTION_UPDATE)
    public void updateModule(HttpRequest req, HttpResponse resp) throws IOException {
        EmailEntity bean = req.getJsonParameter(EmailEntity.class, "bean");    
        
        long time = System.currentTimeMillis();
        bean.setCtime(time);
        
        emailService.update(bean);    
        resp.finishJson(OssRetCodes.retResult(OssRetCodes.SUCCESS));
    }
    
    @WebAction(url = "/interface/email/create", actionid = ACTION_CREATE)
